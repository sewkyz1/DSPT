...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_type == "peft":
    ...
    elif train_type == "vipt":
        # param_dicts = [
        #     {"params": [p for n, p in net.named_parameters() if ("prompt" in n) and p.requires_grad]},
        #     {
        #         "params": [p for n, p in net.named_parameters() if "ft_linear" in n and p.requires_grad],
        #         "lr": cfg.TRAIN.LR * 0.1,
        #     },
        # ]
        # for n, p in net.named_parameters():
        #     if "prompt" not in n and "qkv_lora" not in n:
        #         p.requires_grad = False
        #     else:
        #         print(n)
        ...
        # # sawm
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if ("prompt" in n) and ("mask" in n) and p.requires_grad]},
            {
                "params": [p for n, p in net.named_parameters() if "ft_linear" in n and p.requires_grad],
                "lr": cfg.TRAIN.LR * 0.1,
            },
