...
class SUTRACK(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        self.params.checkpoint = "/data/disk1/keyuze/SUTRACK/SUTrack-main/weights/SUTRACK_ep0180.pth.tar"
        # self.params.checkpoint = "/data/disk1/keyuze/SUTRACK/SUTrack-main/checkpoints/train/sutrack/rgbt/sutrack_b224_0000005_10000_sawm/SUTRACK_ep0005.pth.tar"
        # self.params.checkpoint = "/data/disk1/keyuze/SUTRACK/SUTrack-main/checkpoints/train/sutrack/sutrack_b224/SUTRACK_ep0010.pth.tar"
        # self.params.checkpoint = "/data/disk1/keyuze/SUTRACK/SUTrack-main/checkpoints/train/sutrack/rgbd/sutrack_b224_0000005_10000_sawm/SUTRACK_ep0010.pth.tar"
        # self.params.checkpoint = "/data/disk1/keyuze/SUTRACK/SUTrack-main/checkpoints/train/sutrack/rgbe/sutrack_b224_0000005_10000_sawm/SUTRACK_ep0005.pth.tar"
        # self.params.checkpoint = '/data/disk1/keyuze/SUTRACK/SUTrack-main/checkpoints/train/sutrack/sutrack_b224_rgbt_only16/SUTRACK_ep0010.pth.tar'
        try:
            miss, unexpect = network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=False)
            print('Load pretrained model from: ' + self.params.checkpoint)
            print(f"missing_keys: {miss}")
            print(f"unexpected_keys: {unexpect}")
        except:
            print('error')
        # print(ada)
        self.cfg = params.cfg
    ...
    def initialize(self, image, info: dict):
        ...
        # template_r = template[:,:3]
        # template_r = torch.cat((template_r, template_r), axis=1)
        # template_dte = template[:,3:]
        # # self.template_list = [template_r, template_dte] * self.num_template
        # self.template_list = [template_r] * self.num_template
        # self.template_list.extend([template_dte] * self.num_template)
        ...
        if self.multi_modal_language:
            if self.use_nlp:
                init_nlp = info.get("init_nlp")  # 应该是rgb+n模态才有的
            else:
                init_nlp = None
    ...
    def track(self, image, info: dict = None):
        ...
        search_list = [search]
        # cv2.imwrite('./draw_fig/search1.png', x_patch_arr[:,:,0:3])


        # # vipt
        # search_r = search[:,:3]
        # search_r = torch.cat((search_r, search_r), axis=1)
        # search_dte = search[:,3:]
        # search_list = [search_r, search_dte]
